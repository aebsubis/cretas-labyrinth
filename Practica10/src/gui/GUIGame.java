...
public class GUIGame extends Canvas implements Runnable, CommandListener{
	...
	private void paintScreen(Graphics graphics) {
		// Desfase vertical ocupado por la barra de estado.
	   	...
	   	Location2D cameraLocation = GUIHandler.getInstance().getCamera();
	   	...
	   	ArrayList objects = GUIHandler.getInstance().getObjects();
	   	int objectSize = objects.size();

	   	// Dibujamos el escenario.
	   	for (int i = 0; i < objectSize; i++) {
	   		// Obtenemos el objeto i.
			GUIObject o = (GUIObject) objects.get(i);
			...
			if(objectLocation.getX() >= xMin && objectLocation.getX() <= xMax && objectLocation.getY() >= yMin && objectLocation.getY() <= yMax) {
				...
				graphics.drawImage(ResourcesHandler.getInstance().getImage(o.getImage()), xBlockSize * (objectLocation.getX() - xMin), yBarSize + (yBlockSize * (objectLocation.getY() - yMin)), Graphics.LEFT | Graphics.TOP);
			}
		}
	}
	...
	protected void keyPressed(int keyCode) {
		GameHandler.getInstance().addEvent(keyCode);
	}
}
